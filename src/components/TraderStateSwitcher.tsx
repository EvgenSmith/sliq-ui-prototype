// Developer-only state switcher to preview the trader's page-level user states
// on /listings and /trader/*. Mirror of LPStateSwitcher (same visual treatment:
// monospace + dashed border + neutral gray). Intentionally NOT brand-styled —
// signals "dev tooling, not user UI". Lives next to NetworkSwitcher in
// AppHeader, but only on Trade routes (Pools routes show LPStateSwitcher).

import { useEffect, useRef, useState } from 'react'
import { TRADER_DEMO_STATES, useTraderDemoState } from '@/lib/traderDemoState'

export function TraderStateSwitcher() {
  const [state, setState] = useTraderDemoState()
  const [open, setOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
  const current = TRADER_DEMO_STATES.find(s => s.id === state) ?? TRADER_DEMO_STATES[2]
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        title="Dev: Trader page state switcher (prototype only)"
      >
        [TR: {current.code}]
      </button>
      {open && (
        ...
        >
          <div className="px-2.5 py-1.5 text-[10px] uppercase font-mono tracking-wider text-gray-400 bg-gray-50 border-b border-gray-200">
            Dev · Trader page state
          </div>
          {TRADER_DEMO_STATES.map(s => (
            <button
              key={s.id}
              type="button"
              onClick={() => {
